...
// Remove the old global scripts related to statistics if any exist from before
$c = preg_replace('/<script>\s*document\.addEventListener\(\'alpine:init\', \(\) => \{[\s\S]*?<\/script>/', '', $c);
...
$startIdx = strpos($c, '<template x-if="activeTab === \'statistics\'">');
...
echo "Injected perfect layout successfully.\n";
