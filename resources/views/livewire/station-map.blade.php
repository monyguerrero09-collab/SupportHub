            ...
            <div class="lg:col-span-4 h-full pb-10">
                ...
                <div class="bg-[#0a0a1a]/80 backdrop-blur-3xl rounded-[2rem] border border-white/5 shadow-[0_20px_50px_rgba(0,0,0,0.5)] h-full flex flex-col overflow-hidden animate-in zoom-in-95 duration-300">
                    ...
                    <div class="p-8 flex-1 overflow-y-auto space-y-4">
                        <h4 class="text-[10px] font-black text-gray-600 uppercase tracking-widest mb-6 border-b border-white/5 pb-2">Activos Vinculados</h4>
                        @forelse($selectedStation['equipment'] as $item)
                        <div class="p-5 bg-white/[0.03] border border-white/5 rounded-2xl group hover:bg-white/10 transition-all">
                            <p class="font-black text-white text-sm uppercase tracking-tight">{{ $item->name ?? 'Equipo #'.$item->id }}</p>
                            <p class="text-[9px] font-bold text-gray-500 uppercase tracking-widest mt-1">{{ $item->model ?? 'N/A' }} | S/N: {{ $item->serial_number ?? 'N/A' }}</p>
                        </div>
                        @empty
